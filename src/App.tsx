...
const workExperience: TimelineGroup[] = [
  {
    ...
    entries: [
      { title: "Full Stack Developer", date: "Jul 2026 - Present" },
      { title: "Senior Python Developer", date: "Jun 2025 - Jul 2026" },
    ],
  },
  {
    organization: "NotBug",
    entries: [
      { title: "Full-Stack Software Engineer", date: "Jan 2024 - Apr 2025" },
    ],
  },
  {
    organization: "TietoEvry",
    entries: [
      { title: "Python Software Engineer", date: "Nov 2022 - Sep 2024" },
    ],
  },
  {
    organization: "Nokia",
    entries: [{ title: "Software Engineer", date: "Jan 2020 - Oct 2022" }],
  },
];

const coursesAndCertifications: TimelineGroup[] = [
  {
    organization: "Google Cloud",
    entries: [
      {
        title: "Professional Cloud Developer",
        date: "Issued Apr 28, 2026",
        href: "https://www.credly.com/badges/40f6e45b-cef3-4de4-aa94-ae26f96b7d44/public_url",
      },
    ],
  },
  {
    organization: "Przeprogramowani",
    entries: [
      {
        title: "10xDevs 2",
  ...
  },
  {
    ...
    entries: [
      {
        title: "AI Devs 2 - Connect GPT-4 with application and automation logic",
...
const featuredProjects: TimelineGroup[] = [
  {
    organization: "Language Learning Buddy",
    entries: [
      {
        title:
          "AI assistant for analyzing sentences in English and Polish, identifying grammar mistakes, and building personalized learning lists",
        href: "https://language-learning-buddy.pl",
        linkLabel: "Visit language-learning-buddy.pl →",
      },
    ],
  },
  {
    ...
    entries: [
      {
        title:
          "Tennis coach website designed to attract new students and highlight coaching style.",
        href: "https://kapusz-tenis.pl",
        linkLabel: "Visit kapusz-tenis.pl →",
      },
    ],
  },
];
...
const aboutHighlights = [
  "I keep communication crisp and progress visible.",
  "I build with empathy for the next developer on the team.",
  "I prefer steady delivery over noisy perfection.",
];
  ...
  return (
    <div className="mt-3">
      {entries.map((entry, index) => {
        ...
        return (
          <div key={entry.title} className="grid grid-cols-[1rem_1fr] gap-x-3">
            <div className="relative flex justify-center pt-1">
              {!isLast && (
                <span
                  className="absolute top-4 bottom-0 left-1/2 w-px -translate-x-1/2 bg-zinc-700/80"
                  aria-hidden="true"
                />
              )}
              <span
                className="relative flex h-4 w-4 items-center justify-center rounded-full bg-zinc-800"
                aria-hidden="true"
              >
            ...
            <div className={isLast ? undefined : "pb-4"}>
              ...
              {entry.href && (
                ...
                >
                  {entry.linkLabel ?? "Show credential →"}
                </a>
              )}
            </div>
          </div>
...
function TimelineTextList({ items }: { items: string[] }) {
  return (
    <div className="mt-6">
      {items.map((item, index) => {
        ...
        return (
          <div key={item} className="grid grid-cols-[1rem_1fr] gap-x-3">
            <div className="relative flex justify-center pt-1">
              {!isLast && (
                <span
                  className="absolute top-4 bottom-0 left-1/2 w-px -translate-x-1/2 bg-zinc-700/80"
    ...
    <div>
      {groups.map((group, index) => (
        <div
          ...
          className={
            index === 0 ? undefined : "mt-6 border-t border-zinc-800/80 pt-6"
          }
        >
          <p className="font-bold">{group.organization}</p>
...
type TechLogo = "python" | "flask" | "django" | "fastapi" | "typescript" | "react";
...
const techStackGroups: TechStackGroup[] = [
  {
    ...
    entries: [
      {
        name: "Python",
        description: "Fast, reliable APIs and automation",
        logo: "python",
      },
      {
        name: "Flask",
        description: "Lightweight services and internal tools",
        logo: "flask",
      },
      {
        name: "Django",
        description: "Product backends with batteries included",
        logo: "django",
      },
      {
        name: "FastAPI",
        description: "Typed, async-friendly API delivery",
        logo: "fastapi",
      },
    ],
  },
  {
    ...
    entries: [
      {
        name: "TypeScript",
        description: "Stable front-ends without regressions",
        logo: "typescript",
      },
      {
        name: "React",
        description: "Smooth user journeys and maintainable UI",
        logo: "react",
      },
    ],
  },
...
function TechLogoIcon({ logo }: { logo: TechLogo }) {
  if (logo === "typescript") {
    return (
      <svg
        className="h-6 w-6"
  ...
  if (logo === "flask") {
    return (
      ...
      >
        <path d="M9 3h6" />
        <path d="M10 3v6.4L5.6 17a3 3 0 0 0 2.6 4.5h7.6a3 3 0 0 0 2.6-4.5L14 9.4V3" />
        <path d="M8 15h8" />
      </svg>
    );
  }
  ...
  if (logo === "django") {
    return (
      <span
        className="flex h-6 w-6 items-center justify-center rounded bg-zinc-500 text-[10px] font-bold text-zinc-950"
        aria-hidden="true"
      >
        dj
      </span>
  ...
  return (
    ...
    >
      ...
      <path d="M13.3 2.6 6.8 13h4.1l-1.8 8.4 7.1-11h-4.4z" fill="#09090b" />
    </svg>
  );
}

function TechStackEntries({ entries }: { entries: TechStackEntry[] }) {
  return (
    <div className="mt-3">
      {entries.map((entry, index) => {
        const isLast = index === entries.length - 1;
        ...
        return (
          <div key={entry.name} className="grid grid-cols-[1rem_1fr] gap-x-3">
            <div className="relative flex justify-center pt-1">
              {!isLast && (
                <span
                  className="absolute top-4 bottom-0 left-1/2 w-px -translate-x-1/2 bg-zinc-700/80"
...
function TechStackGroups({ groups }: { groups: TechStackGroup[] }) {
  return (
    <div>
      {groups.map((group, index) => (
        <div
        ...
        >
          ...
          <TechStackEntries entries={group.entries} />
        </div>
      ))}
    </div>
  );
      ...
      <main id="main-content" className="pt-24 pb-16 px-4 md:px-6 relative">
        <div className="max-w-6xl mx-auto space-y-16">
          ...
          <section className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
            ...
            <div
              id="about"
              className="card order-1 md:order-none md:col-span-2 scroll-mt-20 reveal"
            >
              ...
              <TimelineTextList items={aboutHighlights} />
            </div>

            {/* Featured Projects */}
            <div
              id="work"
              className="card order-4 md:order-none scroll-mt-20 reveal"
              style={{ "--stagger": "100ms" } as React.CSSProperties}
            >
              ...
              <TimelineGroups groups={featuredProjects} />
            </div>

            {/* Skills Card */}
            <div
              className="card order-3 md:order-none reveal"
              style={{ "--stagger": "200ms" } as React.CSSProperties}
            >
              <Code2 className="w-5 h-5 mb-4 text-zinc-500" />
              <h2 className="text-2xl font-semibold mb-4">Tech Stack</h2>
              <TechStackGroups groups={techStackGroups} />
            </div>

            {/* Experience */}
            <div className="card order-2 md:order-none reveal">
              <BookOpen className="w-5 h-5 mb-4 text-zinc-500" />
              <h2 className="text-2xl font-semibold mb-4">Experience</h2>
              <TimelineGroups groups={workExperience} />
            </div>

            <div
              className="card order-5 md:order-none reveal"
              style={{ "--stagger": "150ms" } as React.CSSProperties}
            >
              ...
              <TimelineGroups groups={coursesAndCertifications} />
            </div>

            {/* GitHub Activity Card */}
            <div
              className="card order-6 md:order-none reveal md:col-span-3"
              style={{ "--stagger": "100ms" } as React.CSSProperties}
            >
            ...
            <div
              id="contact"
              className="card order-7 md:order-none col-span-1 md:col-span-3 scroll-mt-20 reveal"
            >
